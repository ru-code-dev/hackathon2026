...
Версия: 1
Дата: 2026-07-28
Статус: логика реализована и покрыта тестами; UI не начат
...
Девять правил, все детерминированные, ноль токенов LLM.
...
| `a11y.name.missing` | error | `iconOnly`, `empty`, `unlabelled` | `4.1.2` | ✗ | ✅ |
...
4. **Нет отдельной оси «A11y» в health score.** `healthScore` остаётся одним числом; a11y-находки входят в него наравне с остальными по severity. Если ось нужна, её надо добавить в `summarySchema`.
5. **APG не извлечён.** Контракт паттернов сейчас — список из 11 ролей внутри `pattern-keyboard.ts` плюс данные `aria-query`. Артефакта `apg-patterns.json` нет.
...
| `name.missing` | Текст из выражения (`{label}`) — виден только литеральный JSX-текст |
