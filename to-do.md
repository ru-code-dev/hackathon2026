...
- [x] **Слой доступности** — 38 проверок: 9 своих правил + 29 канонических из `eslint-plugin-jsx-a11y`; у каждой находки `a11y.fix`, у четырёх — готовый дифф
- [x] **Экран «Доступность»** — пятый пункт левого меню, `architecture.md` §7.6. Экраны 1–4 не тронуты
...
- [ ] Расширить оракул: 7 a11y-ожиданий из 115 — для a11y метрика по-прежнему статистически пустая
- [ ] Прогон на стороннем продуктовом проекте: все три найденных ложных срабатывания нашлись так, а не тестами
- [ ] Поднять грань `a11y` в `Problem`/`ProblemCard` — на «Плане работ» контекст доступности сейчас не виден (отложено сознательно: экраны 1–4 не трогаем)
- [ ] Дымовой рендер-тест экрана: логика раскладки покрыта (`dashboard-a11y.test.ts`), сам React-компонент проверялся SSR-прогоном вручную
